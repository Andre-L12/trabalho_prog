...
tjogo AtualizaVetorColisoes(tjogo jogo, int x, int pista_galinha){
    int i, j, c = 0,col,l;
    for (i = 0; i < jogo.qtd_pistas; i++){
        if (i == pista_galinha){
            for (j = 0; j < obtemNumCarrosPista(jogo.pistas[i]); j++){ 
...
//arquivos
void ImprimeInicializacao(tjogo jogo, FILE *diretorio){
    FILE *pInicialização;
...
void ImprimeResumo(tjogo jogo,FILE *diretorio){
    FILE *pResumo;
    int i;
...
void ImprimeRanking(tjogo jogo, FILE *diretorio){
...
void ImprimeEstatistica(tjogo jogo, FILE *diretorio){
    FILE *pEstatistica; 
...
void ImprimeHeatmap(tjogo jogo,FILE *diretorio){
    FILE *pMapaCalor;
...
int main(int argc, char *argv[])
{
    ...
    do{
        ...
        //aumenta interação - movimenta carros - conta movimentos w s
        ...
        {   jogo=AtualizaVetorColisoes(jogo, x, pista_galinha);
