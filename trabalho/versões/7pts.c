...
void ImprimeEstatistica(tjogo jogo, FILE *pEstatisca){
    ...
    fprintf(pEstatisca,"Numero total de movimentos: %d\n",jogo.n_movimentosW+jogo.n_movimentoS);
    fprintf(pEstatisca,"Altura maxima que a galinha chegou: %d\n",maxAltura);
    if (jogo.n_colisoes == 0) {
    fprintf(pEstatisca,"Altura maxima que a galinha foi atropelada: 0\n");
    fprintf(pEstatisca,"Altura minima que a galinha foi atropelada: 0\n");
    } else {
    int maxAtro = linhas - jogo.galinha.maximo_atropelamento;
    int minAtro = linhas - jogo.galinha.min_atropelamento;
    fprintf(pEstatisca,"Altura maxima que a galinha foi atropelada: %d\n", maxAtro);
    fprintf(pEstatisca,"Altura minima que a galinha foi atropelada: %d\n", minAtro);
    }
    fprintf(pEstatisca,"Numero de movimentos na direcao oposta: %d\n",jogo.n_movimentoS);
...
int main(int argc, char *argv[])
{
    ...
    ImprimeEstatistica(jogo,pEstatistica);
    if (galinhaAtravesou(jogo))
    {
