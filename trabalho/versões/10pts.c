...
tpista moveCarroPista(tpista pista, int j, int x, int set)
{
    pista.posicao_carros[j] = moveCarro(pista.posicao_carros[j], x, set); // recebe a pista, o carro e o valor do x do carro set altera ou soma
    return pista;
}
...
int MovimentaGalinhaJogo(tjogo jogo, char movimento)
{
    if (movimento == 'w')
    {
...
{
    int i, j, p;
    // 1 for obtem a qtd de pistas
    for (i = 0; i < ObtemQtdPistasJogo(jogo); i++)
    {
        if (obtemDirecaoPista(jogo.pistas[i]) == 'E')//se pista começar com E a velocidade é negativa
        {
            for (j = 0; j < obtemNumCarrosPista(jogo.pistas[i]); j++)//2 for percorre os carros 
            {
                for (p = 0; p < obtemVelocidadePista(jogo.pistas[i]); p++)//soma nos caros a velocidade
                {
                    jogo.pistas[i] = moveCarroPista(jogo.pistas[i], j, -1, 0);
                    if (obtemXCarroPista(jogo.pistas[i], j) < 0)//se passar volta para o maximo da matriz
                    {
                        jogo.pistas[i] = moveCarroPista(jogo.pistas[i], j, jogo.largura_mapa - 1, 1);
                    }
                }
            }
        }
        else if (obtemDirecaoPista(jogo.pistas[i]) == 'D')//velocidade positiva
        {
            for (j = 0; j < obtemNumCarrosPista(jogo.pistas[i]); j++)//percorre carrros
            {
                for (p = 0; p < obtemVelocidadePista(jogo.pistas[i]); p++)//soma nos carros a velocidade
                {
                    jogo.pistas[i] = moveCarroPista(jogo.pistas[i], j, 1, 0);
                    if (obtemXCarroPista(jogo.pistas[i], j) > ObtemLarguraMapaJogo(jogo) - 1)//se passar do limite volta para o incio
                    {
                        jogo.pistas[i] = moveCarroPista(jogo.pistas[i], j, 0, 1);
                    }
                }
            }
        }
...
tjogo AumentaInteracao(tjogo jogo, char movimento)
{
    ...
    jogo = MovimentaCarros(jogo);//movimenta os carros pos cada iteracao
    return jogo;
}
...
tjogo RealizaJogo(tjogo jogo)
{
    ...
    do
    {
        do // so recebe os movimentos w s ' '
        {
            scanf("%c", &movimento);
            if (movimento == '\n')
                continue;
        ...
        int y = MovimentaGalinhaJogo(jogo, movimento); // preve o movimento
        int x = obtemXGalinha(jogo.galinha);
...
int main(int argc, char *argv[])
{
    ...
    // REALIZA JOGO -  valta com o ultimo estado do jogo
