...
{
    int posicao_inicial = ((jogo.qtd_pistas * 3) - 1) - 1;
...
void inicializarMatriz(int linhas, int colunas, char matriz[linhas][colunas], tjogo jogo){
    int i, j,p,k;
    for (i = 0; i < linhas; i++)
    {
        if (!((1 + i) % 3))
    ...
    for (i = 0; i < 2; i++) {        // linhas da galinha
        for (j = 0; j < 3; j++) {    // colunas da galinha
            int posY = (jogo.galinha.y-1) +i;         // linha da galinha
            int posX = jogo.galinha.x + (j - 1);   // centraliza no X

            // Só desenha se estiver dentro dos limites
            if (posY >= 0 && posY < linhas &&
                posX >= 0 && posX < jogo.largura_mapa) {
...
tjogo Lejogo(FILE *pConfig_inicial, FILE *pPersonagens)
{
    ...
    inicializarMatriz(linhas, jogo.largura_mapa, jogo.matriz, jogo);
    return jogo;
}
void Cabelho(tjogo jogo,FILE *pSaida){
    int vidas=obtemVidasGalinha(jogo.galinha);
    int interacoes=ObtemInteracoesJogo(jogo);
    printf("Pontos: %d | Vidas: %d | Iteracoes: %d\n",jogo.pontuacao,vidas,interacoes);
